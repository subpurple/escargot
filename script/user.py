...
def main(email: str, username: str, *, oldmsn: bool = False, yahoo: bool = False, bucpaim: bool = False) -> None:
    ...
    with conn.session() as sess:
        ...
        set_passwords(user, pw, support_old_msn=oldmsn, support_yahoo=yahoo, support_bucp_aim=bucpaim)
        sess.add(user)
...
def set_passwords(user: User, pw: str, *, support_old_msn: bool = False, support_yahoo: bool = False,
                  support_bucp_aim: bool = False) -> None:
    ...
    if support_bucp_aim:
        pw_md5_salt = hash.gen_salt()
        user.set_front_data('aim', 'pw_md5_salt', pw_md5_salt)

        pw_md5_old = hash.hasher_md5bucp.encode(pw, salt=pw_md5_salt)
        user.set_front_data('aim', 'pw_md5_old', pw_md5_old)

        pw_md5_new = hash.hasher_md5bucp.encode(pw, 1, salt=pw_md5_salt)
        user.set_front_data('aim', 'pw_md5_new', pw_md5_new)
