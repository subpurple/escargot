...
from util.hash import hasher, hasher_md5, hasher_md5crypt, hasher_md5bucp
...
class UserService:
    ...
    def aim_login_flap(self, username: str, roasted_pwd: bytes) -> bool:
        ...
        def roast(password: bytes, key: bytes) -> bytes:
            chars = cycle(key)
    ...
    def _get_uncached(self, uuid: str) -> Optional[User]:
        with self._conn.session() as sess:
            ...
            return User(dbuser.id, dbuser.uuid, dbuser.email, dbuser.username, dbuser.verified, status, dbuser.settings,
                        dbuser.date_created, dbuser.date_login)
    ...
    def get_oim_single(self, user: User, uuid: str, *, mark_read: bool = False) -> Optional[OIM]:
        ...
        oim = OIM(
            json_oim['uuid'], json_oim['run_id'], json_oim['from'], json_oim['from_username'],
            json_oim['from_friendly']['friendly_name'],
            user.email, iso_parser.parse(json_oim['sent']),
            json_oim['message']['text'], json_oim['message']['utf8'],
            headers=json_oim['headers'],
            from_friendly_encoding=json_oim['from_friendly']['encoding'],
            from_friendly_charset=json_oim['from_friendly']['charset'],
    ...
    def save_oim(
            ...
            from_friendly: Optional[str] = None, from_friendly_charset: str = 'utf-8',
        ...
        oim = OIM(
            oim_json['uuid'], oim_json['run_id'], oim_json['from'], oim_json['from_username'],
    ...
    def _get_groupchat_uncached(self, chat_id: str) -> Optional[GroupChat]:
        with self._conn.session() as sess:
            ...
            groupchat = GroupChat(
                dbgroupchat.chat_id, dbgroupchat.name, dbgroupchat.owner_id, dbgroupchat.owner_uuid,
                dbgroupchat.owner_friendly,
            ...
            for dbgroupchatmembership in dbgroupchatmemberships:
                ...
                groupchat.memberships[head.uuid] = GroupChatMembership(
                    ...
                    inviter_name=dbgroupchatmembership.inviter_name,
                    invite_message=dbgroupchatmembership.invite_message,
                )
            ...
            for dbgroupchat in dbgroupchats:
                if dbgroupchat.chat_id in self._groupchat_cache_by_chat_id:
                    ...
                    dbgroupchatmembership = sess.query(DBGroupChatMembership).filter(
                        DBGroupChatMembership.chat_id == dbgroupchat.chat_id,
                        DBGroupChatMembership.member_uuid == user.uuid
                    ).one_or_none()
    ...
    def save_groupchat_batch(self, to_save: List[Tuple[str, GroupChat]]) -> None:
        with self._conn.session() as sess:
            ...
            for chat_id, groupchat in to_save:
                ...
                dbgroupchatmemberships = sess.query(DBGroupChatMembership).filter(
                    DBGroupChatMembership.chat_id == chat_id)
                ...
                for membership in groupchat.memberships.values():
                    dbgroupchatmembership = sess.query(DBGroupChatMembership).filter(
                        DBGroupChatMembership.chat_id == chat_id,
                        DBGroupChatMembership.member_uuid == membership.head.uuid
                    ).one_or_none()
                    if dbgroupchatmembership is None:
                        dbgroupchatmembership = DBGroupChatMembership(
            ...
            for user, detail in to_save:
                ...
                for c in detail.contacts.values():
                    ...
                    dbusercontact.locations = {
                        location.type: {
                            'name': location.name, 'street': location.street, 'city': location.city,
